...
define([
    ...
    return declare("MetaGrid.widget.MetaGrid", [_WidgetBase, _TemplatedMixin], {
        ...
        getOtherEntityForReference: function (entity, reference) {
            ...
            var entities = Object.keys(mx.meta.getMap());
        ...
        generateHeader: function (headerRow, columns, entity, systemColumns) {
            ...
            for (var index = 0; index < columns.length; index++) {
                var title = columns[index], tooltip = entity.getAttributeType(title), isReference = ['ObjectReference', "ObjectReferenceSet"].indexOf(tooltip) > -1, isAssociation = title.split('.').length > 1, isSortColumn = this.sortAttr == title, a = isAssociation ? title.split('.').reverse()[0] : mxui.dom.create('a', { 'class': 'sort-header ' +
                        (isSortColumn ? (this.sortDirection) : '') }, title), header = mxui.dom.create('th', {
        ...
        _generateGenLinks: function (entities, entityClass, tableRow, guid, index) {
            ...
            for (var index_1 = 0; index_1 < entities.length; index_1++) {
                var div = mxui.dom.create('div', { class: "meta-grid-" + entityClass }, entities[index_1]);
                dojo.place(div, cellDiv);
                dojo.connect(div, "onclick", dojo.hitch(this, this._fetchDataForGrid, null, '//' + entities[index_1] + '[id=\'' + guid + '\']', entities[index_1]));
            }
        ...
        _updatePage: function (pageNumber) {
            ...
            for (var index = 0; index < rows.length; index++) {
                var row = rows[index], from = (this.currentPage * this.pageSizeAttribute.get()), to = from + parseInt(this.pageSizeAttribute.get());
        ...
        getEntityNameIfAtRootLevel: function () {
        ...
        _renderLines: function (objects, entityName, overrideEntity, columns, entity, aggregates, headerRow, tbody) {
            var systemColumns = ['changedDate', 'createdDate', 'System.owner', 'System.changedBy'], odd = true, columnIndex = columns.length, objectMap = mx.meta.getMap(), objectKeys = Object.keys(objectMap);
            for (var idx = 0; idx < objectKeys.length; idx++) {
                var key = objectKeys[idx], objectEntity = objectMap[key];
            ...
            for (var objectIndex = 0; objectIndex < objects.length; objectIndex++) {
                var object = objects[objectIndex], row = mxui.dom.create('tr', { class: "meta-grid-row " + (odd ? "odd" : "even"), index: objectIndex + 1 });
                ...
                for (var colIndex = 0; colIndex < columns.length; colIndex++) {
                    var name = columns[colIndex], date = object.isDate(name) ? (new Date(object.get(name))) : null, attr = object.get(name), classes = "meta-grid-cell meta-grid-col-" + colIndex + "meta-attr-" + name, tooltip = attr, selectorEntity = this.getOtherEntityForReference(entity, name);
        ...
        _renderGrid: function (objects, aggregates, overrideEntity) {
            ...
            var table = mxui.dom.create('table', { class: 'meta-grid-table' + (objects.length == 1 ? ' single' : '') }), thead = mxui.dom.create('thead', { class: "meta-grid-thead" }), tbody = mxui.dom.create('tbody', { class: "meta-grid-tbody" }), headerRow = mxui.dom.create('tr', { class: "meta-grid-header-row" });
            ...
            var entityNameDefault = objects.length ? objects[0].getEntity() : "!dead end!";
            var entityName = this.getEntityNameIfAtRootLevel() || overrideEntity || entityNameDefault;
            ...
            if (objects.length > 0) {
                var entity = mx.meta.getMap()[entityName], columns = this.associationsAttribute.get() ? entity.getAttributes() : entity.getAttributesWithoutReferences();
        ...
        _handleValidation: function (validations) {
            this._clearValidations();
            var validation = validations[0], message = validation.getReasonByAttribute(this.xpath);
            if (this.readOnly) {
        ...
        _fetchAttributes: function (attributes) {
            if (this._contextObj && (attributes.length)) {
                ...
                if (!this[attribute]) {
                    ...
                    var parts = this.codeAttribute.split('/'), attribute = parts.pop();
...
//# sourceMappingURL=MetaGrid.js.map